...
void BSP_LCD_Message (int device, int line, int col, char *string, unsigned int value){
  // Your Code Here
	...
	//Prints string followed by numerical value
	
	//top screen line = (0-11) -> y parameter for DrawString method
	//col -> x parameter for DrawString method 0 (left edge) 20 (right edge)
	//*string -> pt parameter for DrawString
	//change value to char* to be able to display
	...
	if(device == 0) {
		y = line;
		x = col;
		//Print string at starting col value followed by integer after
		BSP_LCD_DrawString(x,y,string,LCD_WHITE);
		x+=2;
...
void BSP_LCD_DrawCrosshair(int16_t x, int16_t y, int16_t color) {
  // Your Code Here
	...
	//lines 5 pixels in length
	
	int16_t w = 9;
	
	//horizontal line of crosshair
	int16_t leftEdge = x - 4;
	
	//vertical line of crosshair
	int16_t topEdge = y - 4;
	
	BSP_LCD_DrawFastHLine(leftEdge,y,w,color);
	
	BSP_LCD_DrawFastVLine(x,topEdge,w,color);
